document.addEventListener("DOMContentLoaded", function () {
    ...
    // Unisci i prodotti dei due carrelli, filtrando solo quelli attivi
    const carrello = [
        ...carrelloPreassemblato.prodotti.filter(p => p.attivo !== false),
        ...carrelloCatalogo.prodotti.filter(p => p.attivo !== false)
    ];
        ...
        carrello.forEach((prodotto, index) => {
            ...
            const prodottoHTML = `
                <div class="prodotto-carrello d-flex align-items-center mb-3" data-index="${index}">
                    ...
                    <button class="btn btn-danger btn-sm disattiva-prodotto">
    ...
    // Gestione della disattivazione dei prodotti dal carrello (delegazione)
    carrelloContainer.addEventListener("click", function (event) {
        if (event.target.classList.contains("disattiva-prodotto") || event.target.closest(".disattiva-prodotto")) {
            const prodottoElement = event.target.closest(".prodotto-carrello");
            const prodottoIndex = parseInt(prodottoElement.getAttribute("data-index"));
            ...
            // Disattiva il prodotto invece di rimuoverlo
            if (prodottoIndex < carrelloPreassemblato.prodotti.length) {
                carrelloPreassemblato.prodotti[prodottoIndex].attivo = false;
                localStorage.setItem("carrelloPreassemblato", JSON.stringify(carrelloPreassemblato));
            } else {
                let updatedIndex = prodottoIndex - carrelloPreassemblato.prodotti.filter(p => p.attivo !== false).length;
                carrelloCatalogo.prodotti[updatedIndex].attivo = false;
    ...
    document.getElementById("svuota-carrello").addEventListener("click", function () {
        // Disattiva tutti i prodotti invece di rimuoverli
        let carrelloPreassemblato = JSON.parse(localStorage.getItem("carrelloPreassemblato")) || { prodotti: [] };
        let carrelloCatalogo = JSON.parse(localStorage.getItem("carrelloCatalogo")) || { prodotti: [] };
